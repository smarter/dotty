...
class Typer extends Namer
  ...
  def typedFunctionValue(tree: untpd.Function, pt: Type)(using Context): Tree = {
    ...
    // contravariant positions. These can be always instantiated without
    // preventing the body of the lambda from typechecking.
    //
    // TODO: This doesn't exclude type variables which appear with a different
    // variance at a later point in the same method call, is this worth
    // supporting?
