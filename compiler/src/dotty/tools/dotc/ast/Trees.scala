...
object Trees {
  ...
  abstract class Instance[T >: Untyped <: Type] { inst =>
    ...
    /** Delegate to FunProto or FunProtoTyped depending on whether the prefix is `tpd` or `untpd`. */
    ...
    def applyOverloaded(
        ...
        expectedType: Type, isContextual: Boolean = false)(using parentCtx: Context): tpd.Tree = {
      ...
      typer.ApplyToUntyped(apply, fun, selected, proto, expectedType).result
    }
