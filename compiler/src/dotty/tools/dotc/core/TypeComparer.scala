...
class TypeComparer(@constructorOnly initctx: Context) extends ConstraintHandling, PatternTypeConstrainer {
   ...
   */
  def lub(tp1: Type, tp2: Type, canConstrain: Boolean = false, isSoft: Boolean = true): Type = /*>|>*/ trace(s"lub(${tp1.show}, ${tp2.show}, canConstrain=$canConstrain)", subtyping, show = true) /*<|<*/ {
    if (tp1 eq tp2) tp1
      ...
      def mergedLub(tp1: Type, tp2: Type): Type = {
        ...
        val tp1w = widen(tp1)
        val tp2w = widen(tp2)
        if ((tp1 ne tp1w) || (tp2 ne tp2w)) lub(tp1w, tp2w, canConstrain, isSoft)
   ...
   */
  final def orType(tp1: Type, tp2: Type, isErased: Boolean = ctx.erasedTypes, isSoft: Boolean = true): Type = {
...
object TypeComparer {
  ...
  def lub(tp1: Type, tp2: Type, canConstrain: Boolean = false, isSoft: Boolean = true)(using Context): Type =
    comparing(_.lub(tp1, tp2, canConstrain, isSoft))
