...
// ---- Symbol creation methods ----------------------------------
  ...
  def newSymbol[N <: Name](
    ...
    val level = if ctx.scope ne null then ctx.scope.nestingLevel else 0
    val sym = new Symbol(coord, ctx.base.nextSymId, level).asInstanceOf[Symbol { type ThisName = N }]
  ...
  = {
    val level = if ctx.scope ne null then ctx.scope.nestingLevel else 0
    val cls = new ClassSymbol(coord, assocFile, ctx.base.nextSymId, level)
