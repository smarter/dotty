    ...
    if !constraint.contains(param) then true
      ...
      val bound =
        if this.isInstanceOf[GadtConstraint] then
          ...
          // Approximate if needed to avoid wildcards and types with a level
          // strictly greater than `level`. Note that level-checking must be
          // performed here and cannot be delayed until instantiation because if
          // we allow level-incorrect bounds, then we might end up reasoning
          // with bad bounds outside of the scope where they are defined. This
          // can lead to level-correct but unsound instantiations as demonstrated
          // by tests/neg/i8900.scala.
          val level = nestingLevel(param)
