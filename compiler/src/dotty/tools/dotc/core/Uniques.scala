...
class Uniques extends WeakHashSet[Type](Config.initialUniquesCapacity) with
...
object Uniques with
  ...
  final class NamedTypeUniques extends WeakHashSet[NamedType](Config.initialUniquesCapacity * 4) with Hashable with
    ...
    def enterIfNew(prefix: Type, designator: Designator, isTerm: Boolean)(using Context): NamedType =
      ...
      if h == NotCached then newType
        ...
        def add() = {
          val elem = newType
          table(bucket) = new Entry(elem, h, oldHead, queue)
          count += 1
  ...
  final class AppliedUniques extends WeakHashSet[AppliedType](Config.initialUniquesCapacity * 2) with Hashable with
